...
export default function AdminTeamDetail() {
    ...
    async function deleteTeam() {
        if (!confirm("Supprimer cette equipe et tous ses projets/taches ?")) return;
        try {
    ...
    async function deleteProject(projectId: string, name: string) {
        if (!confirm(`Supprimer le projet "${name}" et toutes ses taches ?`)) return;
        try {
                ...
                <div className="lg:col-span-2 bg-slate-900 rounded-xl border border-slate-800 p-5">
                    ...
                    {projects.length === 0 ? (
                        <p className="text-slate-400 text-sm text-center py-6">
                            Aucun projet dans cette equipe.
                        </p>
                    ) : (
                        <div className="space-y-2">
