...
export default function Teams() {
    ...
    async function deleteTeam(id: string, name: string) {
        if (
            !confirm(
                `Supprimer l'équipe "${name}" et tous ses projets/tâches ? Cette action est irreversible.`,
            )
        )
            return;
        try {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-6">Équipes ({teams.length})</h1>

            {teams.length === 0 ? (
                <div className="bg-slate-900 rounded-xl border border-slate-800 p-8 text-center">
                    <p className="text-slate-400">Aucune équipe.</p>
                </div>
            ) : (
