...
export default function TeamDetail() {
  ...
  async function invite(e: React.FormEvent) {
    ...
    try {
      ...
      setInviteStatus({ type: "error", msg: err instanceof Error ? err.message : "Erreur" });
    }
  }
  ...
  return (
    <div>
      <Link to="/teams" className="inline-flex items-center gap-1 text-sm text-muted hover:text-white transition-colors">
      ...
      <div className="grid gap-6 lg:grid-cols-3 items-start">
        <div className="lg:col-span-1 bg-surface-card rounded-xl border border-border p-5">
          ...
          {showInvite && (
            <form onSubmit={invite} className="bg-surface-elevated border border-border rounded-lg p-4 mb-4 space-y-3">
              <input
                type="email"
                ...
                required
              />
              <button type="submit" className="inline-flex items-center gap-2 px-4 py-2 bg-accent hover:bg-accent-hover rounded-lg text-sm font-medium transition-colors">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {inviteStatus && (
                <p className={`text-xs p-2 rounded ${inviteStatus.type === "success" ? "text-emerald-400 bg-emerald-400/10" : "text-red-400 bg-red-400/10"}`}>
                  {inviteStatus.msg}
                </p>
              )}
            </form>
          ...
          <div className="space-y-2">
            {members.map((m) => (
              <div key={m.id} className="flex items-center gap-3 bg-surface-elevated p-3 rounded-lg border border-border">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium truncate">{m.name ?? "Membre"}</p>
        ...
        <div className="lg:col-span-2 bg-surface-card rounded-xl border border-border p-5">
          ...
          {showNewProject && (
            <form onSubmit={createProject} className="bg-surface-elevated border border-border rounded-lg p-4 mb-4 space-y-3">
              <input
                type="text"
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-surface text-white rounded-lg border border-border focus:border-accent focus:outline-none text-sm transition-colors"
              />
              <button type="submit" className="px-4 py-2 bg-accent hover:bg-accent-hover rounded-lg text-sm font-medium transition-colors">
                Creer le projet
              </button>
            </form>
          )}
          ...
          {projects.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted text-sm">Aucun projet. Creez-en un pour commencer.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {projects.map((project) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium group-hover:text-accent transition-colors">{project.name}</h3>
