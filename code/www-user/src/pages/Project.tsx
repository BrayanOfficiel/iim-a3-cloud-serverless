...
const COLUMNS: { key: Task["status"]; label: string; color: string; icon: JSX.Element }[] = [
  {
    key: "todo",
    label: "A faire",
...
export default function Project() {
  ...
  async function createTask(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/projects/${projectId}/tasks`, {
        method: "POST",
        body: JSON.stringify({ name: newTaskName, description: newTaskDesc || null }),
      });
      setNewTaskName("");
      setNewTaskDesc("");
      setShowForm(false);
  ...
  async function updateTask(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/tasks/${editingTask.id}`, {
        method: "PATCH",
        body: JSON.stringify({ name: editingTask.name, description: editingTask.description }),
      });
      setEditingTask(null);
      await loadData();
    } catch (err) {
  ...
  return (
    <div>
      {project && (
        <Link to={`/teams/${project.teamId}`} className="inline-flex items-center gap-1 text-sm text-muted hover:text-white transition-colors">
      ...
      {showForm && (
        <form onSubmit={createTask} className="bg-surface-card border border-border rounded-xl p-4 mb-6 space-y-3">
          <input
            type="text"
