      ...
      <div className="max-w-lg">
        <div className="bg-surface-card rounded-xl border border-border p-6">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              <label htmlFor="profile-name" className="block text-sm text-neutral-300 mb-1">Nom</label>
              <input
                id="profile-name"
                type="text"
                value={name}
            ...
            <div>
              <label htmlFor="profile-email" className="block text-sm text-neutral-300 mb-1">Adresse e-mail</label>
              <input
                id="profile-email"
                type="email"
                value={user.email}
              ...
              />
              <p className="text-xs text-neutral-600 mt-1">L'adresse e-mail ne peut pas etre modifiee.</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="block text-sm text-neutral-300 mb-1">Role</span>
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
                  </svg>
                  <p className="text-sm text-muted">{user.role === "admin" ? "Administrateur" : "Utilisateur"}</p>
                </div>
              </div>
              <div>
                <span className="block text-sm text-neutral-300 mb-1">Membre depuis</span>
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {message && (
              <div className={`flex items-center gap-2 text-sm p-3 rounded-lg ${message.type === "success" ? "text-emerald-400 bg-emerald-400/10 border border-emerald-400/20" : "text-red-400 bg-red-400/10 border border-red-400/20"}`}>
