...
invitationsRouter.post(
  ...
  async (c) => {
    ...
    // Recuperer le nom de l'inviteur et de l'équipe
    const [team] = await sql`SELECT name
                                 FROM teams
                                 WHERE id = ${teamId}`;
    ...
    const [invitation] = await sql`
            ...
            VALUES (${teamId}, ${email}, ${userId}) RETURNING id, team_id, invited_email, invited_by, status, created_at
        `;
...
invitationsRouter.post("/invitations/:id/accept", async (c) => {
  ...
  const [invitation] = await sql`
        UPDATE invitations
        SET status = 'accepted'
        WHERE id = ${id} RETURNING id, team_id, invited_email, invited_by, status, created_at
    `;
